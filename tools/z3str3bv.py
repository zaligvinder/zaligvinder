...
def run (eq,timeout,ploc,wd):
    ...
    myerror = ""
    try:
        out = subprocess.check_output ([path,"smt.string_solver=z3str3","dump_models=true",smtfile],timeout=timeout,stderr=subprocess.STDOUT).decode().strip()
