...
def run (eq,timeout,ploc,wd):
    path = ploc.findProgram ("Z3PortProbe3")
    if not path:
    ...
    time = timer.Timer ()
    try:
        out = subprocess.check_output ([path,"smt.string_solver=z3str3","dump_models=true","smt.str.fixed_length_iterations=5","smt.str.pre_milliseconds=0",smtfile],timeout=timeout).decode().strip()
...
def addRunner (addto):
    addto['z3str4-np_len-abs-5_str3'] = run
