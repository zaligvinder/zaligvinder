...
def run (eq,timeout,ploc,wd,solver="1",param="60"):
    ...
    smtfile = os.path.join (tempd,"cvc5_out.smt")
    ...
    time = timer.Timer ()
    try:
        out = subprocess.check_output ([path,"--lang","smtlib2.6","--no-interactive","--strings-exp","--dump-models","--tlimit-per",str(timeout)+"000",smtfile],timeout=int(timeout)).decode().strip()
