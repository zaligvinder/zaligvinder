...
def run (eq,timeout,ploc,wd):
    ...
    smtfile = os.path.join (tempd,"z3str3_out.smt")
    ...
    time = timer.Timer ()
    try:
        out = subprocess.check_output ([path,"smt.string_solver=z3str3","smt.arith.solver=2","dump_models=true",smtfile],timeout=timeout)
