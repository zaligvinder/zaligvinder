...
def run (eq,timeout,ploc,wd):
    ...
    time = timer.Timer ()
    try:
        out = subprocess.check_output ([path,"smt.string_solver=z3str3","smt.str.tactic=1","smt.str.fixed_length_iterations=5","dump_models=true",smtfile],timeout=timeout).decode().strip()
